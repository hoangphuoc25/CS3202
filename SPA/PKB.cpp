            ...
            DesignEnt yType, int y) const
{
    ...
    set<int>::iterator it;
    set<string> uses = stmtBank->get_node(y)->get_uses();
...
{
    return !affectsBip_X_Y_get_int_Y_from_int_X
        (ENT_ASSIGN,ENT_ASSIGN,x).empty();
}

bool PKB::affectsBip_X_Y_smth_int_Y(DesignEnt yType, int y) const
{
    return !affectsBip_X_Y_get_int_X_from_int_Y
        (ENT_ASSIGN,ENT_ASSIGN,y).empty();
}
